...
public class IOLocalDate {
	...
	private static final String REGEX_DATE = "^(0[1-9]|1[0-2])/(0[1-9]|[12][\\d]|3[01])/(20[0-2][0-4]|19[\\d][\\d])$";
	...
	/**
	 * Use to input date type LocalDate
	 * 
	 * @param
	 * @return LocalDate
	 * @throws BirthdayException
	 */
	public static LocalDate input(String date) throws DateException {
		String[] arrDate;
		if (date.matches(REGEX_DATE)) {
			arrDate = date.split("/");
			if (checkDate(arrDate)) {
				return LocalDate.parse(arrDate[2] + "-" + arrDate[0] + "-" + arrDate[1]);
			}
		}
		throw new DateException("Date not matched validate (mm-dd-yyyy) and in 1900-2024!");
	}
